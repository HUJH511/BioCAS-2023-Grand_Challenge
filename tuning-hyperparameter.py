...
# --------------------Tune traning loop --------------------#
def tuneTrain(config, train_dl, val_dl):
    network = configCNN(
...
if __name__ == "__main__":
    ...
    network = configCNN(
        num_class=num_classes, input_fdim=N_F_BIN, input_tdim=INPUT_X_DIM
    )
    param_size = 0
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(
                tuneTrain, train_dl=dataloader["train"], val_dl=dataloader["val"]
            ),
    ...
    test_network = configCNN(
        num_class=num_classes,
        input_fdim=N_F_BIN,
        input_tdim=INPUT_X_DIM,
        c1=best_result.config["c1"],
        c2=best_result.config["c2"],
        slen=best_result.config["slen"],
